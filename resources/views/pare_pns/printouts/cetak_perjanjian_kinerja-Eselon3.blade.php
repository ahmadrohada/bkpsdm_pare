...
<body>
	...
	@php
		$arrayForTable = [];
		foreach ($program_list as $dbValue) {
			$temp = [];
			$temp['program_label'] 				= $dbValue['program_label'];
			$temp['indikator_program_label']	= $dbValue['ind_program_label'];
			...
			@foreach ($arrayForTable as $id=>$values) :
				...
				<tr>
					@if ($key == 0) :
						...
						<td align='left' class='garis' rowspan={{ count($values) }}>{{ $value['program_label'] }}</td>
						@php $no++; @endphp
					@endif
						<td align='left' class='garis' >{{  $value['indikator_program_label'] }}</td>
