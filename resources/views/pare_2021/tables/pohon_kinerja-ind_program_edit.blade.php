...
<div class="box box-program div_ind_program_list" hidden>
	...
	<div class="box-body table-responsive">
		...
		<div class="toolbar">
			...
			<span  data-toggle="tooltip" title="Create Indikator Kegiatan"><a class="btn btn-info btn-xs create_ind_program" ><i class="fa fa-plus" ></i> Indikator Kegiatan</a></span>
...
@include('pare_2021.modals.renja_ind_program')
...
$('#ind_program_table').DataTable({
	...
	$(document).on('click','.create_ind_program',function(e){
		$('.modal-ind_program').find('h4').html('Create Indikator Kegiatan');
	...
	$(document).on('click','.edit_ind_program',function(e){
		...
		$.ajax({
				...
				success	: function(data) {
					...
					$('.modal-ind_program').find('h4').html('Edit Indikator Kegiatan');
