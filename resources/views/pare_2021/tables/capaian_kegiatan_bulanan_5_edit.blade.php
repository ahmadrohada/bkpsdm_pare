...
<!--====================== KEGIATAN BULANAN LIST =========================================== -->
		<div class="box box-primary" id='kegiatan_bulanan'>
			<div class="box-header with-border">
				<h1 class="box-title">
					List Realisasi Kegiatan Bulanan JFT
				</h1>
									...
									{ data: "label", name:"label",
										"render": function ( data, type, row ) {
											if ( (row.realisasi_kegiatan_bulanan_id) <= 0 ){
												return "<span class='text-danger'>"+row.kegiatan_bulanan_label+"</span>";
											}else{
	...
	function show_modal_create(kegiatan_bulanan_id){
		$.ajax({
				...
				success	: function(data) {
					...
					$('.modal-realisasi_kegiatan_bulanan').find('.penanggung_jawab').html("-");
					...
					$('.modal-realisasi_kegiatan_bulanan').find('.kegiatan_bulanan_label').html(data['label']);
					$('.modal-realisasi_kegiatan_bulanan').find('.pelaksana').html(data['pelaksana']);
					$('.modal-realisasi_kegiatan_bulanan').find('.kegiatan_bulanan_output').html(data['output']);
					$('.modal-realisasi_kegiatan_bulanan').find('.kegiatan_bulanan_satuan').html(data['satuan']);
					$('.modal-realisasi_kegiatan_bulanan').find('.kegiatan_bulanan_target').html(data['target']);
					$('.modal-realisasi_kegiatan_bulanan').find('.kegiatan_bulanan_target').val(data['target']);

					$('.modal-realisasi_kegiatan_bulanan').find('h4').html('Add Realisasi Kegiatan Bulanan');
					$('.modal-realisasi_kegiatan_bulanan').find('.btn-submit').attr('id', 'submit-save_jft');
