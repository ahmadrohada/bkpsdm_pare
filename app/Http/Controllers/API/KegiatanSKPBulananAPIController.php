...
class KegiatanSKPBulananAPIController extends Controller {
    ...
    public function KegiatanBulananDetail(Request $request)
    {
        ...
        $rencana_aksi = array(
            ...
            'penanggung_jawab'              => Pustaka::capital_string($x->RencanaAksi->IndikatorKegiatanSKPTahunan)?($x->RencanaAksi->IndikatorKegiatanSKPTahunan->KegiatanSKPTahunan->SubKegiatan->PenanggungJawab->jabatan):"",
            'kegiatan_tahunan_label'        => ($x->RencanaAksi->IndikatorKegiatanSKPTahunan)?$x->RencanaAksi->IndikatorKegiatanSKPTahunan->KegiatanSKPTahunan->label:"",
            'kegiatan_tahunan_target'       => ($x->RencanaAksi->IndikatorKegiatanSKPTahunan)?$x->RencanaAksi->IndikatorKegiatanSKPTahunan->KegiatanSKPTahunan->target:"",
            ...
            'kegiatan_tahunan_cost'         => number_format( ($x->RencanaAksi->IndikatorKegiatanSKPTahunan)?$x->RencanaAksi->IndikatorKegiatanSKPTahunan->KegiatanSKPTahunan->cost,'0',',','.' : "0"),
    ...
    public function KegiatanBulanan4(Request $request)
    {
        ...
        //$rencana_aksi = $this->TraitKegiatanBulananJFU($request->skp_bulanan_id); 
        ...
        $datatables = Datatables::of($dt)
        ->addColumn('skp_bulanan_id', function ($x) use($skp_bulanan_id){
            return $skp_bulanan_id;
    ...
    public function KegiatanBulanan2(Request $request) 
    {
        ...
        $skp_bln = SKPBulanan::WHERE('id',$request->skp_bulanan_id)->first();
