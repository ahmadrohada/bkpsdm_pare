...
class SKPBulananAPIController extends Controller {
    ...
    public function PersonalSKPBulananList(Request $request)
    {
        ...
        $SKPBulanan = SKPBulanan::WHERE('skp_bulanan.pegawai_id',$id_pegawai)
                        //SKP tahunan
                        ->leftjoin('db_pare_2018_demo.skp_tahunan', function($join){
    ...
    public function SKPDSKPBulanan_list(Request $request)
    {
        ...
        $dt = \DB::table('db_pare_2018_demo.renja AS renja')
                    ...
                    ->join('db_pare_2018_demo.perjanjian_kinerja AS pk', function($join){
                        $join   ->on('pk.renja_id','=','renja.id');
                    })
                    ->join('db_pare_2018_demo.skp_tahunan AS skp_tahunan', function($join){
                        $join   ->on('pk.id','=','skp_tahunan.perjanjian_kinerja_id');
                    }) 
                    ->rightjoin('db_pare_2018_demo.skp_bulanan AS skp_bulanan', function($join){
