...
class TPPReportAPIController extends Controller
{
    ...
    public function Select2CetakPeriodeList(Request $request)
    {
        ...
        $data       = Periode::
                        ...
                        rightjoin('pare_2018_demo.tpp_report AS tpp', function ($join) {
    ...
    public function cetakTPPReportData(Request $request)
    {
        ...
        $p = TPPReport::WHERE('tpp_report.id', $tpp_report_id)
            ...
            ->join('pare_2018_demo.formula_hitung_tpp AS frm', function ($join) {
    ...
    protected function TPPReportDataDetail(Request $request)
    {
        ...
        $x = TPPReportData::
            ...
            //CAPAIAN SKP
            ->leftjoin('pare_2018_demo.capaian_bulanan AS capaian', function ($join) {
    ...
    protected function TPPReportDataEdit(Request $request)
    {
        ...
        $x = TPPReportData::
            ...
            //TPP Report
            ->leftjoin('pare_2018_demo.tpp_report AS tpp_report', function ($join) {
    ...
    public function Store(Request $request)
    {
        ...
        if ($st_kt->save()) {
            ...
            //SKP Bulanan 
            ->leftjoin('pare_2018_demo.skp_bulanan AS skp', function ($join) use($bulan_lalu){
                $join->on('skp.pegawai_id', '=', 'tb_pegawai.id');
                $join->where('skp.bulan', '=', $bulan_lalu) ;
                //$join->where('skp.skp_tahunan_id', '=', 'skp_tahunan.id') ;
            })
            //SKP TAHUNAN 
            ->leftjoin('pare_2018_demo.skp_tahunan AS skp_tahunan', function ($join) use($periode_id){
                $join->on('skp.skp_tahunan_id', '=', 'skp_tahunan.id');
            })
             //RENJA
             ->leftjoin('pare_2018_demo.renja AS renja', function ($join) use($periode_id){
