...
class CapaianTriwulanAPIController extends Controller {
    ...
    public function CreateConfirm(Request $request)
	{
        ...
        $cp_status = SKPTahunan::WHERE('skp_tahunan.id',$skp_tahunan_id)
                            //CAPAIAN TRIWULAN I
                            ->rightjoin('pare_2018_demo.capaian_triwulan AS triwulan', function($join)  use($triwulan){
    ...
    public function PersonalCapaianTriwulanList(Request $request)
    {
        ...
        $SKPTahunan = SKPTahunan::WHERE('skp_tahunan.pegawai_id',$id_pegawai)
                        ...
                        //CAPAIAN TRIWULAN I
                        ->leftjoin('pare_2018_demo.capaian_triwulan AS triwulan1', function($join){
                            $join   ->on('triwulan1.skp_tahunan_id','=','skp_tahunan.id');
                            $join   ->where('triwulan1.triwulan','=','1');
                        })
                         //CAPAIAN TRIWULAN II
                        ->leftjoin('pare_2018_demo.capaian_triwulan AS triwulan2', function($join){
                            $join   ->on('triwulan2.skp_tahunan_id','=','skp_tahunan.id');
                            $join   ->where('triwulan2.triwulan','=','2');
                        })
                        //CAPAIAN TRIWULAN III
                        ->leftjoin('pare_2018_demo.capaian_triwulan AS triwulan3', function($join){
                            $join   ->on('triwulan3.skp_tahunan_id','=','skp_tahunan.id');
                            $join   ->where('triwulan3.triwulan','=','3');
                        })
                         //CAPAIAN TRIWULAN IV
                        ->leftjoin('pare_2018_demo.capaian_triwulan AS triwulan4', function($join){
