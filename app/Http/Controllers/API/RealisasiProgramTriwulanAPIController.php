...
class RealisasiProgramTriwulanAPIController extends Controller {
    ...
    public function RealisasiProgramTriwulan(Request $request) 
    {
        ...
        $dt = Tujuan:: 
                        ...
                        ->leftjoin('db_pare_2018_demo.realisasi_program_triwulan AS realisasi_program', function($join) use ( $capaian_id ){
                        ...
                        ->leftjoin('db_pare_2018_demo.realisasi_indikator_program_triwulan AS realisasi_indikator', function($join) use ( $capaian_id ){
    ...
    public function Destroy(Request $request)
    {
        ...
        if ( $st_kt->delete()){
            ...
            $data_uing = IndikatorProgram::WHERE('program_id',Input::get('program_id'))
                                            ...
                                            ->join('db_pare_2018_demo.realisasi_indikator_program_triwulan AS realisasi_indikator', function($join) use($capaian_id) {
                                                $join   ->on('realisasi_indikator.indikator_program_id','=','renja_indikator_program.id');
                                                $join   ->WHERE('realisasi_indikator.capaian_id','=',  $capaian_id );
