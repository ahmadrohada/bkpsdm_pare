...
use App\Models\RealisasiProgramTahunan;
use App\Models\RealisasiIndikatorProgramTahunan;
...
class RealisasiProgramTahunanAPIController extends Controller {


    public function RealisasiProgramTahunan(Request $request) 
    {
        ...
        $dt = Tujuan:: 
                        ...
                        ->leftjoin('db_pare_2018.realisasi_program_tahunan AS realisasi_program', function($join) use ( $capaian_id ){
                        ...
                        ->leftjoin('db_pare_2018.realisasi_indikator_program_tahunan AS realisasi_indikator', function($join) use ( $capaian_id ){
    ...
    public function AddRealisasiProgramTahunan(Request $request)
    {
        ...
        $ind_program = array(
            ...
            'realisasi_indikator_program_tahunan_id'    => $x->realisasi_indikator_program_id,
            'realisasi_program_tahunan_id'    => $x->realisasi_program_id,
    ...
    public function Store(Request $request)
    {
        ...
        $st_kt    = new RealisasiIndikatorProgramTahunan;
        ...
        if ( $st_kt->save()){
             ...
             $rkt    = RealisasiProgramTahunan::WHERE('capaian_id','=',Input::get('capaian_tahunan_id'))
                                                ->WHERE('program_id','=',Input::get('program_id'))
                                                ->count();

            //jiki belum ada add new
            if ( $rkt == 0 ) {
            $rkt_save    = new RealisasiProgramTahunan;
        ...
        //jika sudah ada update saja
            ...
            $rkt_update                     = RealisasiProgramTahunan::find(Input::get('realisasi_program_tahunan_id'));
    ...
    public function Update(Request $request)
    {
            ...
            $messages = [
                'realisasi_indikator_program_tahunan_id.required'  => 'Harus diisi',
                'capaian_tahunan_id.required'                      => 'Harus diisi',
                'program_id.required'                               => 'Harus diisi',
                'indikator_program_id.required'                     => 'Harus diisi',
        ...
        $validator = Validator::make(
                        Input::all(),
                        array(
                            'realisasi_indikator_program_tahunan_id'   => 'required',
                            'capaian_tahunan_id'                       => 'required',
                            'program_id'                                => 'required',
                            'indikator_program_id'                      => 'required',
        ...
        $st_update                          = RealisasiIndikatorProgramTahunan::find(Input::get('realisasi_indikator_program_tahunan_id'));
    ...
    public function Destroy(Request $request)
    {
        ...
        $st_kt    = RealisasiIndikatorProgramTahunan::find(Input::get('realisasi_indikator_id'));
        if (is_null($st_kt)) {
        ...
        if ( $st_kt->delete()){
            ...
            $data_uing = IndikatorProgram::WHERE('program_id',Input::get('program_id'))
                                            ...
                                            ->join('db_pare_2018.realisasi_indikator_program_tahunan AS realisasi_indikator', function($join) use($capaian_id) {
                                                $join   ->on('realisasi_indikator.indikator_program_id','=','renja_indikator_program.id');
                                                $join   ->WHERE('realisasi_indikator.capaian_id','=',  $capaian_id );
            ...
            if ( $data_uing === 0 ){
                $del_ah    = RealisasiProgramTahunan::find(Input::get('realisasi_program_id'));
                $del_ah->delete();
            }
