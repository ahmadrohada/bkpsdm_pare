...
use App\Models\RealisasiSasaranTahunan;
use App\Models\RealisasiIndikatorSasaranTahunan;
...
class RealisasiSasaranTahunanAPIController extends Controller {


    public function RealisasiSasaranTahunan(Request $request) 
    {


        $capaian_id     = $request->capaian_pk_tahunan_id;
        ...
        $dt = Tujuan:: 
                        ...
                        //INDIKATOR SASARAN
                        ->leftjoin('db_pare_2018_demo.renja_indikator_sasaran AS indikator_sasaran', function($join){
                            $join   ->on('indikator_sasaran.sasaran_id','=','sasaran.id');
                        })
                         
                       
                        //LEFT JOIN TERHADAP REALISASI SASARAN
                        ->leftjoin('db_pare_2018_demo.realisasi_sasaran_tahunan AS realisasi_sasaran', function($join) use ( $capaian_id ){
                        ...
                        ->leftjoin('db_pare_2018_demo.realisasi_indikator_sasaran_tahunan AS realisasi_indikator', function($join) use ( $capaian_id ){
    ...
    public function AddRealisasiSasaranTahunan(Request $request)
    {
        ...
        $x = IndikatorSasaran::
                            leftjoin('db_pare_2018_demo.renja_sasaran AS renja_sasaran', function($join) {
        ...
        $ind_sasaran = array(
            ...
            'realisasi_indikator_sasaran_tahunan_id'    => $x->realisasi_indikator_sasaran_id,
            'realisasi_sasaran_tahunan_id'    => $x->realisasi_sasaran_id,
    ...
    public function Store(Request $request)
    {
        ...
        $st_kt    = new RealisasiIndikatorSasaranTahunan;

        $st_kt->indikator_sasaran_id    = Input::get('indikator_sasaran_id');
        $st_kt->capaian_id              = Input::get('capaian_tahunan_id');
        ...
        if ( $st_kt->save()){
             ...
             $rkt    = RealisasiSasaranTahunan::WHERE('capaian_id','=',Input::get('capaian_tahunan_id'))
                                                ->WHERE('sasaran_id','=',Input::get('sasaran_id'))
                                                ->count();

            //jiki belum ada add new
            if ( $rkt == 0 ) {
            $rkt_save    = new RealisasiSasaranTahunan;
            $rkt_save->capaian_id              = Input::get('capaian_tahunan_id');
        ...
        //jika sudah ada update saja
            ...
            $rkt_update                     = RealisasiSasaranTahunan::find(Input::get('realisasi_sasaran_tahunan_id'));
    ...
    public function Update(Request $request)
    {
            ...
            $messages = [
                'realisasi_indikator_sasaran_tahunan_id.required'  => 'Harus diisi',
                'capaian_tahunan_id.required'                      => 'Harus diisi',
                'sasaran_id.required'                               => 'Harus diisi',
                'indikator_sasaran_id.required'                     => 'Harus diisi',
        ...
        $validator = Validator::make(
                        Input::all(),
                        array(
                            'realisasi_indikator_sasaran_tahunan_id'   => 'required',
                            'capaian_tahunan_id'                       => 'required',
                            'sasaran_id'                                => 'required',
                            'indikator_sasaran_id'                      => 'required',
        ...
        $st_update                          = RealisasiIndikatorSasaranTahunan::find(Input::get('realisasi_indikator_sasaran_tahunan_id'));
    ...
    public function Destroy(Request $request)
    {
        ...
        $st_kt    = RealisasiIndikatorSasaranTahunan::find(Input::get('realisasi_indikator_id'));
        if (is_null($st_kt)) {
        ...
        if ( $st_kt->delete()){
            ...
            $data_uing = IndikatorSasaran::WHERE('sasaran_id',Input::get('sasaran_id'))
                                            ...
                                            ->join('db_pare_2018_demo.realisasi_indikator_sasaran_tahunan AS realisasi_indikator', function($join) use($capaian_id) {
                                                $join   ->on('realisasi_indikator.indikator_sasaran_id','=','renja_indikator_sasaran.id');
                                                $join   ->WHERE('realisasi_indikator.capaian_id','=',  $capaian_id );
            ...
            if ( $data_uing === 0 ){
                $del_ah    = RealisasiSasaranTahunan::find(Input::get('realisasi_sasaran_id'));
                $del_ah->delete();
            }
