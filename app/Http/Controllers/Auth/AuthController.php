	...
	*/
	...
	public function validator(array $data)
	{
		return Validator::make($data, [
			...
			//'nip' 		=> 'required|nip|max:255|unique:users',
	...
	public function sendEmail(User $user)
	{
		...
		\Mail::queue('nips.activateAccount', $data, function($message) use ($user) {
    ...
    public function getSocialHandle( $provider )
    {
        ...
        //CHECK IF USERS NIP ADDRESS IS ALREADY IN DATABASE
        $user_check = User::where('nip', '=', $user->nip)->first();
        if(!empty($user_check))
        {
            $social_user = $user_check;
        }
        else // USER IS NOT IN DATABASE BASED ON nip ADDRESS
        {
            ...
            if(empty($same_social_id))
            {
                ...
                $new_social_user->nip            	= $user->nip;
                $name 								= explode(' ', $user->name);
				if ($user->nip) {
					$new_social_user->name         	= $user->nip;
				} else {
				...
				$the_activation_code 				= str_random(60) . $user->nip;
				...
				// GET GRAVATAR
				$new_social_user->gravatar          = Gravatar::get($user->nip);
