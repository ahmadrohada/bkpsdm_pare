<?php
    ...
    HTML::macro('show_username', function() {
        //$the_username = (Auth::user()->name === Auth::user()->nip) ? ((is_null(Auth::user()->first_name)) ? (Auth::user()->name) : (Auth::user()->first_name)) : (((is_null(Auth::user()->name)) ? (Auth::user()->nip) : (Auth::user()->name)));
        $the_username = Auth::user()->nama;
